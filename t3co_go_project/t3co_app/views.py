        ...
        if form.is_valid():
            try:
                ...
                if results and results.get("success"):
                    ...
                    return redirect("parameter_analysis_results", analysis_id=analysis.id)
                else:
                    error_msg = results.get(
            ...
            try:
                ...
                if selection_method == "file_upload":
                    ...
                    for vehicle_file in vehicle_files:
                        path = default_storage.save(f"temp/{vehicle_file.name}", vehicle_file)
                        vehicle_paths.append(os.path.join(settings.MEDIA_ROOT, path))
                    
                    for scenario_file in scenario_files:
                        path = default_storage.save(f"temp/{scenario_file.name}", scenario_file)
                    ...
                    vehicle_configurations = form.cleaned_data.get("vehicle_configurations")
                ...
                if results and results.get("success"):
                    ...
                    form_data = form.cleaned_data
                    chart_data = prepare_comparison_charts(results, vehicle_files, form_data)
                    ...
                    context = {
                        ...
                        "chart_data": chart_data,
                        "form_data": form_data,
                        "vehicle_names": [f.name for f in vehicle_files] if vehicle_files else [],
                        "scenario_names": [f.name for f in scenario_files] if scenario_files else [],
                        "form": VehicleComparisonForm(),
                    }
                    return render(request, "t3co_app/comparison_results.html", context)
                else:
                    error_msg = results.get("error", "Unknown error occurred during comparison")
...
def prepare_comparison_charts(results, vehicle_files=None, comparison_form_data=None):
    ...
    try:
        ...
        group_by_primary = comparison_form_data.get("group_by_primary", "fuel_type") if comparison_form_data else "fuel_type"
        group_by_secondary = comparison_form_data.get("group_by_secondary", "none") if comparison_form_data else "none"
        chart_type = comparison_form_data.get("chart_type", "stacked_bar") if comparison_form_data else "stacked_bar"
        comparison_metrics = comparison_form_data.get("comparison_metrics", ["total_cost"]) if comparison_form_data else ["total_cost"]
        include_breakdown = comparison_form_data.get("include_cost_breakdown", True) if comparison_form_data else True
        ...
        cost_categories = {
            ...
            "fuel_converter_cost_dol": {"label": "Fuel Converter Cost", "color": "#228B22"},
            "fuel_storage_cost_dol": {"label": "Fuel Storage Cost", "color": "#8B4513"},
            "motor_control_power_elecs_cost_dol": {"label": "Motor/Power Electronics", "color": "#1874CD"},
            ...
            "total_maintenance_cost_dol": {"label": "Maintenance Cost", "color": "#DAA520"},
            "total_fuel_cost_dol": {"label": "Fuel Cost", "color": "#4682B4"},
            "fueling_dwell_labor_cost_dol": {"label": "Fueling/Dwell Labor", "color": "#CD2626"},
            "discounted_downtime_oppy_cost_dol": {"label": "Downtime Opportunity Cost", "color": "#8B0000"},
            "payload_capacity_cost_dol": {"label": "Payload Capacity Cost", "color": "#CD8C95"},
        }
        ...
        for idx, scenario_result in enumerate(comparison_data):
            ...
            secondary_group_value = parsed_components.get(group_by_secondary, "None") if group_by_secondary != "none" else "None"
        ...
        if include_breakdown and chart_type == "stacked_bar":
            chart_data["tco_stacked_comparison"] = create_stacked_comparison_chart(
                grouped_data, cost_categories, labels, group_by_primary, group_by_secondary
            )
        ...
        for metric in comparison_metrics:
            if metric == "total_cost":
                chart_data["total_cost_comparison"] = create_metric_comparison_chart(
                    grouped_data, "discounted_tco_dol", "Total Cost of Ownership", 
                    group_by_primary, chart_type
                )
            elif metric == "cost_per_mile":
                chart_data["cost_per_mile_comparison"] = create_metric_comparison_chart(
                    grouped_data, "cost_per_mile", "Cost per Mile", 
                    group_by_primary, chart_type
                )
            elif metric == "fuel_efficiency":
                chart_data["fuel_efficiency_comparison"] = create_metric_comparison_chart(
                    grouped_data, "fuel_efficiency_mpgge", "Fuel Efficiency (MPGGE)", 
                    group_by_primary, chart_type
                )
...
def create_stacked_comparison_chart(grouped_data, cost_categories, labels, primary_group, secondary_group):
    ...
    return {
        ...
        "options": {
            "responsive": True,
            "plugins": {
                "title": {"display": True, "text": f"TCO Cost Breakdown Comparison (Grouped by {primary_group.replace('_', ' ').title()})"},
            ...
            "scales": {
                "x": {"stacked": True, "title": {"display": True, "text": "Vehicle-Scenario Combinations"}},
                "y": {
                    "stacked": True,
                    "beginAtZero": True,
                    "title": {"display": True, "text": "Cost ($)"},
...
def create_metric_comparison_chart(grouped_data, metric_key, metric_label, group_by, chart_type):
    ...
    chart_config = {
        "type": "bar" if chart_type in ["stacked_bar", "grouped_bar"] else chart_type.replace("_", ""),
        "data": {
            "labels": labels,
            "datasets": [{
                "label": metric_label,
...
def create_matrix_comparison_chart(grouped_data, primary_group, secondary_group, metrics):
    ...
    return {
        ...
        "data": {
            ...
            "datasets": [{
                "label": "Total Cost",
                "data": [
                    sum(scenario["data"].get("discounted_tco_dol", 0) for scenario in scenarios) / len(scenarios)
        ...
        "options": {
            "responsive": True,
            "plugins": {
                "title": {"display": True, "text": f"Matrix Comparison: {primary_group.replace('_', ' ').title()} vs {secondary_group.replace('_', ' ').title()}"},
            },
            "scales": {
                "y": {
                    "beginAtZero": True,
...
def create_comparison_summary_chart(grouped_data, metrics):
    ...
    for scenarios in grouped_data.values():
        avg_cost = sum(scenario["data"].get("discounted_tco_dol", 0) for scenario in scenarios) / len(scenarios)
    ...
    return {
        ...
        "data": {
            ...
            "datasets": [{
                "data": avg_costs,
                "backgroundColor": ["#1976d2", "#4caf50", "#ff9800", "#f44336", "#9c27b0", "#607d8b"],
                "borderWidth": 2,
            }]
        },
        "options": {
            "responsive": True,
            "plugins": {
                "title": {"display": True, "text": "Average Cost Distribution by Group"},
                "legend": {"position": "bottom"},
                "tooltip": {
                    "callbacks": {
...
def prepare_ledger_analysis_charts(results):
    ...
    try:
        ...
        print(f"Key metrics - Total Cost: ${total_cost:,.2f}, Cost/mile: ${cost_per_mile:.3f}")
        print(f"Cost breakdown keys: {list(cost_breakdown.keys())}")
        
        # Extract detailed cost components
        purchase_cost = cost_breakdown.get("purchase_cost", details.get("purchase_cost", 0))
        fuel_cost = cost_breakdown.get("fuel_cost", details.get("fuel_cost", 0))
        maintenance_cost = cost_breakdown.get("maintenance_cost", details.get("maintenance_cost", 0))
        insurance_cost = cost_breakdown.get("insurance_cost", details.get("insurance_cost", 0))
        
        # Additional costs from details
        depreciation = details.get("depreciation", 0)
        registration_cost = details.get("registration_cost", 0)
        
        print(f"Cost components - Purchase: ${purchase_cost:,.2f}, Fuel: ${fuel_cost:,.2f}, Maintenance: ${maintenance_cost:,.2f}")
        
        # Create comprehensive cost breakdown for stacked bar chart
        cost_components = {}
        
        if purchase_cost > 0:
            cost_components["Vehicle Purchase"] = {"value": purchase_cost, "color": "#1976d2", "category": "capital"}
        if depreciation > 0:
            cost_components["Depreciation"] = {"value": depreciation, "color": "#2196f3", "category": "capital"}
        if registration_cost > 0:
            cost_components["Registration"] = {"value": registration_cost, "color": "#03dac6", "category": "capital"}
            
        if fuel_cost > 0:
            cost_components["Fuel"] = {"value": fuel_cost, "color": "#ff5722", "category": "operating"}
        if maintenance_cost > 0:
            cost_components["Maintenance"] = {"value": maintenance_cost, "color": "#795548", "category": "operating"}
        if insurance_cost > 0:
            cost_components["Insurance"] = {"value": insurance_cost, "color": "#9c27b0", "category": "operating"}
        
        # If we have a total cost but no breakdown, create estimates
        if not cost_components and total_cost > 0:
            print("Creating estimated breakdown from total cost...")
            cost_components = {
                "Vehicle Purchase": {"value": total_cost * 0.40, "color": "#1976d2", "category": "capital"},
                "Fuel": {"value": total_cost * 0.35, "color": "#ff5722", "category": "operating"},
                "Maintenance": {"value": total_cost * 0.15, "color": "#795548", "category": "operating"},
                "Insurance": {"value": total_cost * 0.07, "color": "#9c27b0", "category": "operating"},
                "Registration": {"value": total_cost * 0.03, "color": "#03dac6", "category": "capital"},
            }
        ...
        capital_total = sum(comp["value"] for comp in cost_components.values() if comp["category"] == "capital")
        operating_total = sum(comp["value"] for comp in cost_components.values() if comp["category"] == "operating")
        
        if capital_total + operating_total > 0:
            chart_data["cost_category_summary"] = {
                "type": "doughnut",
                "data": {
                    "labels": ["Capital Costs", "Operating Costs"],
                    "datasets": [{
                        "data": [capital_total, operating_total],
        ...
        # Key metrics chart
        fuel_efficiency = details.get("fuel_efficiency", raw_results.get("fuel_efficiency", 0))
        annual_miles = details.get("annual_miles", raw_results.get("annual_miles", 100000))
        vehicle_life_years = details.get("vehicle_life_years", raw_results.get("vehicle_life_years", 7))
        ...
        cost_efficiency = max(0, min(100, 100 - (cost_per_mile * 100)))  # Lower cost = higher score
        fuel_score = min(100, (fuel_efficiency / 10) * 100) if fuel_efficiency > 0 else 50
        reliability_score = 85  # Placeholder
        range_score = min(100, (annual_miles / 100000) * 100) if annual_miles > 0 else 80
        maintenance_score = max(0, min(100, 100 - (maintenance_cost / total_cost * 100))) if total_cost > 0 else 70
        
        chart_data["performance_radar"] = {
            "type": "radar",
            "data": {
                "labels": ["Cost Efficiency", "Fuel Economy", "Reliability", "Range Capability", "Maintenance"],
                "datasets": [{
                    "label": "Performance Score",
                    "data": [cost_efficiency, fuel_score, reliability_score, range_score, maintenance_score],
                    "borderColor": "#1976d2",
...
def prepare_parameter_analysis_charts(results):
    ...
    try:
        ...
        # Main TCO Stacked Bar Chart - Comprehensive Cost Breakdown
        if tco_breakdown:
            ...
            for cost_key, cost_value in tco_breakdown.items():
                if cost_value > 0:  # Only include non-zero costs
                    if cost_key in cost_categories:
                        category = cost_categories[cost_key]
                        if cost_key in ['glider_cost_dol', 'fuel_converter_cost_dol', 'fuel_storage_cost_dol', 
                                       'motor_control_power_elecs_cost_dol', 'battery_cost_dol', 'plug_cost_dol', 'purchase_tax_dol']:
                            capital_costs[category["label"]] = {"value": cost_value, "color": category["color"]}
                        elif cost_key in ['total_fuel_cost_dol', 'total_maintenance_cost_dol', 'insurance_cost_dol']:
                            operating_costs[category["label"]] = {"value": cost_value, "color": category["color"]}
                        else:
                            opportunity_costs[category["label"]] = {"value": cost_value, "color": category["color"]}
        else:
            ...
            if total_cost > 0:
                ...
                capital_costs = {
                    ...
                    "Fuel Converter Cost": {"value": total_cost * 0.20, "color": "#228B22"},
                    "Battery Cost": {"value": total_cost * 0.15, "color": "#7EC0EE"},
                }
                operating_costs = {
                    "Fuel Cost": {"value": total_cost * 0.30, "color": "#4682B4"},
                    "Maintenance Cost": {"value": total_cost * 0.10, "color": "#DAA520"},
                }
        ...
        if all_costs:  # Only create chart if we have cost data
            chart_data["tco_stacked_breakdown"] = {
                ...
                "options": {
                    "responsive": True,
                    "plugins": {
                        "title": {"display": True, "text": "T3CO Cost Breakdown - Stacked Analysis"},
                        "legend": {"position": "bottom", "labels": {"usePointStyle": True}},
                        "tooltip": {
                            "callbacks": {
                                "label": "function(context) { return context.dataset.label + ': $' + context.parsed.y.toLocaleString(); }"
                    ...
                    "scales": {
                        "x": {"stacked": True, "title": {"display": True, "text": "Cost Categories"}},
                        "y": {
                            "stacked": True,
                            "beginAtZero": True,
                            "title": {"display": True, "text": "Cost ($)"},
        ...
        # Capital vs Operating vs Opportunity Cost Summary
        total_capital = sum(cost_info["value"] for cost_info in capital_costs.values())
        total_operating = sum(cost_info["value"] for cost_info in operating_costs.values()) 
        total_opportunity = sum(cost_info["value"] for cost_info in opportunity_costs.values())

        chart_data["cost_category_summary"] = {
            "type": "doughnut",
            "data": {
                "labels": ["Capital Costs", "Operating Costs", "Opportunity Costs"],
                "datasets": [
                    {
                        "data": [total_capital, total_operating, total_opportunity],
            ...
            "options": {
                "responsive": True,
                "plugins": {
                    "title": {"display": True, "text": "Cost Category Distribution"},
                    "legend": {"position": "bottom"},
                    "tooltip": {
                        "callbacks": {
        ...
        metrics = {
            ...
            "Range (miles/100)": kpis.get("range_miles", 0) / 100,  # Scale for visibility
        }
        ...
        annual_maintenance_timeline = results.get("annual_maintenance_costs_timeline", [])

        if not annual_costs_timeline:
            # Generate basic timeline if not available
            annual_costs_timeline = [annual_cost] * vehicle_life_years
            annual_fuel_timeline = [results.get("fuel_cost", 0) / vehicle_life_years] * vehicle_life_years
            annual_maintenance_timeline = [results.get("maintenance_cost", 0) / vehicle_life_years] * vehicle_life_years
        ...
        performance_metrics = {
            "Fuel Efficiency": min(100, (kpis.get("fuel_efficiency_mpgge", 7) / 15) * 100),  # Normalize to 0-100
            "Range Capability": min(100, (kpis.get("range_miles", 600) / 1000) * 100),  # Normalize to 0-100
            "Cost Effectiveness": min(100, (50000 / max(1, cost_per_mile * 100000)) * 100),  # Normalize
            "Payload Efficiency": min(100, (1 / max(0.1, kpis.get("payload_impact", 1))) * 100),  # Normalize
            "Uptime": min(100, max(0, 100 - (kpis.get("downtime_hours", 0) / 10))),  # Normalize downtime
        }
